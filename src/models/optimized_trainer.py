...
class OptimizedModelTrainer:
    ...
    def train_xgboost_tuned(
        ...
        # Base model
        xgb_model = xgb.XGBRegressor(random_state=42, tree_method="hist", verbosity=0)
        ...
        logger.info(
            f"XGBoost metrics: RMSE={metrics['rmse']:.6f}, R²={metrics['r2']:.4f}"
        )
    ...
    def train_random_forest_tuned(
        ...
        logger.info(
            f"Random Forest metrics: RMSE={metrics['rmse']:.6f}, R²={metrics['r2']:.4f}"
        )
    ...
    def train_gradient_boosting(
        ...
        logger.info(
            f"Gradient Boosting metrics: RMSE={metrics['rmse']:.6f}, R²={metrics['r2']:.4f}"
        )
    ...
    def train_ensemble(
        ...
        sorted_models = sorted(
            individual_models.items(), key=lambda x: x[1]["metrics"]["r2"], reverse=True
        )[:3]
        ...
        logger.info(
            f"Ensemble metrics: RMSE={metrics['rmse']:.6f}, R²={metrics['r2']:.4f}"
        )
    ...
    def plot_predictions(
        self, y_test: np.ndarray, results: Dict[str, Dict], save_path: str = None
    ):
        ...
        for idx, (name, result) in enumerate(results.items()):
            ...
            axes[idx].plot(
                y_pred, label="Predicted", marker="x", alpha=0.7, linewidth=2
            )
            axes[idx].set_title(
    ...
    def save_best_model(
        self, name: str, model: Any, metrics: Dict, params: Dict = None
    ):
...
def main():
    ...
    results["XGBoost_Tuned"] = trainer.train_xgboost_tuned(
        X_train, y_train, X_test, y_test
    )
    print(
        f"✓ RMSE: {results['XGBoost_Tuned']['metrics']['rmse']:.6f}, "
        f"R²: {results['XGBoost_Tuned']['metrics']['r2']:.4f}"
    )

    print("\n[3] Training Random Forest with hyperparameter tuning...")
    results["RandomForest_Tuned"] = trainer.train_random_forest_tuned(
        X_train, y_train, X_test, y_test
    )
    print(
        f"✓ RMSE: {results['RandomForest_Tuned']['metrics']['rmse']:.6f}, "
        f"R²: {results['RandomForest_Tuned']['metrics']['r2']:.4f}"
    )

    print("\n[4] Training Gradient Boosting...")
    results["GradientBoosting"] = trainer.train_gradient_boosting(
        X_train, y_train, X_test, y_test
    )
    print(
        f"✓ RMSE: {results['GradientBoosting']['metrics']['rmse']:.6f}, "
        f"R²: {results['GradientBoosting']['metrics']['r2']:.4f}"
    )

    print("\n[5] Creating ensemble model...")
    results["Ensemble"] = trainer.train_ensemble(
        X_train, y_train, X_test, y_test, results
    )
    print(
        f"✓ RMSE: {results['Ensemble']['metrics']['rmse']:.6f}, "
    ...
    trainer.plot_model_comparison(
        comparison_df, "reports/optimization/model_comparison.png"
    )
    trainer.plot_predictions(
        y_test, results, "reports/optimization/all_predictions.png"
    )
    print("✓ Saved plots")
