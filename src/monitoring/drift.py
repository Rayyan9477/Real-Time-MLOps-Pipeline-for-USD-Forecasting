...
class DriftDetector:
    ...
    def __init__(
        self, significance_level: float = 0.05, z_score_threshold: float = 3.0
    ):
        """
    ...
    def fit(self, reference_data: pd.DataFrame) -> "DriftDetector":
        ...
        logger.info(
            f"Computed reference statistics for {len(self.reference_statistics)} features"
        )
        return self

    def detect_ks_drift(
        self, current_data: pd.DataFrame, features: Optional[List[str]] = None
        ...
        logger.info(
            f"KS drift detection: {drift_count}/{len(results)} features drifted"
        )
    ...
    def detect_point_drift(
        ...
        drift_ratio = (
            len(drifted_features) / total_features if total_features > 0 else 0.0
        )
        drift_detected = drift_ratio > 0.2  # More than 20% features drifted

        return drift_detected, drift_ratio, drifted_features

    def compute_drift_metrics(self, current_data: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        avg_wasserstein = (
            np.mean(wasserstein_distances) if wasserstein_distances else 0.0
        )

        return {
            "drift_ratio": (
                drifted_features / total_features if total_features > 0 else 0.0
            ),
            "num_drifted_features": drifted_features,
            "total_features": total_features,
            "avg_wasserstein_distance": avg_wasserstein,
            "max_wasserstein_distance": (
                max(wasserstein_distances) if wasserstein_distances else 0.0
            ),
        }
